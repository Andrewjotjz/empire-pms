...
const ProjectForm = () => {
  ...
  const [projectState, setProjectState] = useState({
    ...
    suppliers: [],
    companies: [], // Fixed: changed from 'companies: null' to array
    area_obj_ref: [],
  })
  ...
  const validateForm = () => {
    ...
    // Validate areas
    if (projectState.area_obj_ref.length === 0) {
      errors.areas = "At least one area must be added"
    } else {
  ...
  // Loading states
  if (isLoadingState || fetchCompanyLoading || addProjectLoading) {
    return (
  ...
  // Error states
  if (errorState || fetchCompanyError || addProjectError) {
    const error = errorState || fetchCompanyError || addProjectError
...
//                   </div>
//                   {/* LEVELS */}
//                   <div className="border-l-2 border-indigo-300 mb-1 p-2 bg-indigo-100 bg-opacity-50">
