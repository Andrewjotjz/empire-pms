...
const NewPaymentForm = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "supplier") {
      const filteredInvoices = invoiceState.filter(
        (invoice) => invoice.supplier._id === value && !invoice.invoice_isarchived,
      )
  ...
  const handleCheckboxChange = (event) => {
    ...
    setTargetCompanies((prevState) => {
      ...
      return updatedCompanies // Return the array directly, not an object
    })
  ...
  const getSelectedCompanyNames = () => {
    ...
    if (targetCompanies.length === 1) {
      const company = companyState.find((c) => c._id === targetCompanies[0]) // targetCompanies[0] is the ID string
  ...
  const handleSubmit = async (e) => {
    ...
    // Check for invoices that already exist in other statements
    const existingInvoices = targetInvoices.filter((invoice) => invoice.payment && invoice.payment !== null)
    ...
    if (unreviewedInvoices.length > 0) {
      const confirmed = window.confirm("You have unapproved invoice(s). Are you sure you want to create this payment?")
      if (!confirmed) return
    }
    ...
    try {
      const updatedPaymentState = {
        ...paymentState,
        companies: targetCompanies, // Add this line
        invoices: formatInvoices(targetInvoices),
        payment_status: calculatePaymentStatus(),
      }

      const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/payment/create`, {
        credentials: "include",
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true)
      try {
        const [suppliersRes, projectsRes, invoicesRes, companiesRes] = await Promise.all([
        ...
        const [suppliersData, projectsData, invoicesData, companiesData] = await Promise.all([
          suppliersRes.json(),
          projectsRes.json(),
          invoicesRes.json(),
          companiesRes.json(),
  ...
  useEffect(() => {
    if (paymentState.supplier && paymentState.period_start_date && paymentState.period_end_date) {
      const filteredInvoices = invoiceState.filter(
        (invoice) =>
          invoice.supplier._id === paymentState.supplier &&
      ...
      // Also, filter invoices by Project and by CompanyID
      const filteredProject = projectState.filter(proj => targetCompanies.some(companyID => companyID === proj.companies))

      const filteredInvoicesByCompany = filteredInvoices.filter(invoice => filteredProject.some(proj => proj._id === invoice.order.project))

      setTargetInvoices(filteredInvoicesByCompany)
    }
  }, [invoiceState, paymentState.supplier, paymentState.period_start_date, paymentState.period_end_date, targetCompanies])
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-black-600 to-black-700 px-6 py-4 bg-dark">
            <h1 className="text-2xl font-bold text-white">Create New Payment</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {/* <FormField label="Company" name="company" required error={formErrors.company}>
                  <select
                    name="company"
                    value={targetCompanies._id}
                      ...
                      {companyState.map((company) => (
                        ...
                        >
                          <input
                            ...
                            name="companies"
                            value={company._id}
                            checked={targetCompanies.some((c) => c === company._id)}
                ...
                {/* <div>
                  ...
                  {formErrors.companies && <p className="mb-4 text-sm text-red-600">{formErrors.companies}</p>}
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {companyState.map((company) => (
                      <div key={company._id} className="flex items-center">
                        ...
                        />
                        <label htmlFor={`company-${company._id}`} className="text-gray-700">
                          {company.company_name}
                        </label>
                      </div>
                    ))}
                  </div>
                  <p className="mt-2 text-sm text-gray-500">Selected: {getSelectedCompanyNames()}</p>
                </div> */}
                ...
                <FormField
                  label="Total Payment Amount (incl. GST)"
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <FormField
                  ...
                  type="date"
                  value={paymentState.period_start_date}
                  onChange={handleChange}
                />
                <FormField
                  ...
                  type="date"
                  value={paymentState.period_end_date}
                  onChange={handleChange}
                />
              </div>
                        ...
                        <td colSpan={7} className="px-6 py-4">
                          <div className="flex flex-col sm:flex-row items-center gap-4">
                            ...
                            >
                              ...
                              {invoiceState
                                .filter(
                                  (invoice) => invoice.supplier._id === paymentState.supplier &&
                                  (invoice.invoice_issue_date.split("T")[0] < paymentState.period_start_date || invoice.invoice_issue_date.split("T")[0] > paymentState.period_end_date) &&
...
//         <form  onKeyDown={(e) => { if (e.key === 'Enter') {e.preventDefault();} }} className='p-4 pt-1' onSubmit={handleSubmit}>
//             <h1 className='text-2xl font-medium m-1 mt-0'>NEW PAYMENT</h1>
//             {/* PAYMENT DETAILS */}
