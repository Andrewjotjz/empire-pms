...
export default function LibraryList({ onUploadClick }) {
  ...
  const handleView = (resource) => {
    console.log("Opening viewer for:", resource)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              {filteredAndSortedResources.map((resource) => {
                ...
                return (
                  <div key={resource.id} className="bg-white border rounded-lg p-6 hover:shadow-md transition-shadow">
                    ...
                    <button
                      onClick={() => handleView(resource)}
                      className="w-full border border-gray-300 text-gray-700 hover:bg-gray-50 py-2 px-4 rounded-lg transition-colors flex items-center justify-center gap-2 mb-3"
                    >
                      <Eye className="h-4 w-4" />
                      View
                    </button>
        ...
        {/* File Viewer Modal */}
        <FileViewer isOpen={isViewerOpen} onClose={() => setIsViewerOpen(false)} file={viewerFile} />
      </div>
    </div>
  )
}
