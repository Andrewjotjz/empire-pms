...
const SupplierDetails = () => {
  ...
  const [isRemovingProjects, setIsRemovingProjects] = useState(false)
  ...
  const handleAddProductClick = () =>
    navigate(`/EmpirePMS/supplier/${id}/products/create`, {
      state: { supplierId: id, supplierName: supplierState.supplier_name },
    })
  ...
  const handleOrderTableClick = (id) => {
    setPurchaseOrderState([]) // Clear purchase order state instead of dispatch
  ...
  const handleRemoveEmployeesConfirm = async () => {
    ...
    setIsRemovingProjects(true)

    try {
      if (projectsToRemoveArray.length > 0) {
        await Promise.all(
          projectsToRemoveArray.map(async (projectID) => {
            const projectRes = await fetch(`${process.env.REACT_APP_API_BASE_URL}/project/${projectID}`, {
              credentials: "include",
              headers: {
                "Content-Type": "application/json",
      ...
      await fetchSupplierDetails()
      setProjectsToRemove(new Set()) // Reset to empty Set
    } catch (error) {
      console.error("Error removing projects:", error)
    } finally {
      setIsRemovingProjects(false)
    }
  }
  ...
  const handleArchive = () => {
    ...
    setSupplierState(updatedState) // Direct state update instead of dispatch
  ...
  const fetchSupplierDetails = useCallback(async () => {
    try {
      ...
      setSupplierState(data) // Direct state update instead of dispatch
  ...
  useEffect(() => {
    const fetchSupplierProducts = async () => {
      try {
        ...
        setProductState(data.filter((prod) => prod.product.product_isarchived === false)) // Direct state update
        setIsLoadingState(false)
      } catch (err) {
        setErrorState(err.message)
        setIsLoadingState(false)
  ...
  useEffect(() => {
    const fetchAllProjects = async () => {
      try {
        ...
        setProjectState(projectsData) // Direct state update instead of dispatch
        setIsLoadingState(false)
      } catch (error) {
            ...
            <tbody>
              {supplierState.projects.map((project, index) => (
                <tr
                  ...
                  key={`projectEmployeesTab-${project._id}`}
                  onClick={() => handleTableClick("project", project._id)}
                >
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 sm:p-6 md:p-8 text-xs sm:text-base">
      <div className="bg-white p-2 sm:p-5 rounded-lg shadow-lg w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-3xl">
        ...
        <div className="grid gap-0 sm:gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
          {Array.isArray(projectState) &&
            projectState.map((project) => (
              <div
                key={`addProjectsPopUp-${project._id}`}
        ...
        <div className="flex flex-col sm:flex-row justify-end mt-0 sm:mt-5 space-y-2 sm:space-y-0 sm:space-x-2 text-xs sm:text-base">
          <button
            className=" bg-gray-300 text-gray-800 hover:bg-gray-400 px-4 py-2 rounded-md font-medium disabled:opacity-50 w-full sm:w-auto"
            onClick={() => setAddProjectListVisible(false)}
          >
            Cancel
          </button>
          <button
            className=" bg-blue-500 text-white hover:bg-blue-600 px-4 py-2 rounded-md font-medium disabled:opacity-50 w-full sm:w-auto"
            onClick={handleAddProjectConfirm}
            disabled={isAddingProjects}
          >
            {isAddingProjects ? `Adding${dots}` : "Confirm"}
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="bg-white p-2 sm:p-5 rounded-lg shadow-lg w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-3xl">
        ...
        {supplierState?.projects && supplierState.projects.length > 0 ? (
          <div className="grid gap-0 sm:gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
            {supplierState.projects.map((project) => (
              <div
                key={`removeProjectPopUp-${project._id}`}
                className="flex items-center space-x-0 sm:space-x-4 p-2 border-b border-gray-200"
              >
                <input
                  className="form-checkbox h-3 w-3 sm:h-5 sm:w-5 text-blue-600"
        ...
        <div className="flex flex-col sm:flex-row justify-end mt-0 sm:mt-5 space-y-2 sm:space-y-0 sm:space-x-2 text-xs sm:text-base">
          ...
          <button
            ...
            disabled={isRemovingProjects}
          >
            {isRemovingProjects ? `Removing${dots}` : "Confirm"}
          </button>
        </div>
      </div>
    </div>
        ...
        <table className="table table-bordered table-hover text-xs">
          ...
          <tbody>
            {productState &&
              filterProducts()
                .filter(
                  (product, index, self) => index === self.findIndex((p) => p.product._id === product.product._id),
                )
                .map((product, index) => (
                  <tr
                    key={index}
                    onClick={() => handleProductTableClick(product.product._id)}
                    className="cursor-pointer"
                  >
  ...
  const supplierPurchaseOrdersTable =
    Array.isArray(purchaseOrderState) && purchaseOrderState.length > 0 ? (
            ...
            <tbody>
              {filterOrders()
                ...
                .map((order, index) => (
                  <tr
                    key={order._id}
                    onClick={() => handleOrderTableClick(order._id)}
  ...
  const archiveModal = (
    <Modal show={showModal} onHide={() => setShowModal(false)}>
      ...
      <Modal.Body>
        {supplierState && supplierState.supplier_isarchived
          ? `Are you sure you want to unarchive this supplier?`
    ...
    <div className="container mt-5">
      <div className="card">
        ...
        <div className="card-body">
          <div>
            <button
              className={`${currentTab === "supplierDetails" ? "border-x-2 border-t-2 p-1 sm:p-2 rounded bg-gray-700 text-white text-xs sm:text-base" : "border-x-2 border-t-2 p-1 sm:p-2 rounded bg-transparent text-black hover:scale-90 transition ease-out duration-50 text-xs sm:text-base"}`}
              onClick={() => setCurrentTab("supplierDetails")}
            >
              Details
            </button>
            <button
              className={`${currentTab === "supplierProductsTable" ? "border-x-2 border-t-2 p-1 sm:p-2 rounded bg-gray-700 text-white text-xs sm:text-base" : "border-x-2 border-t-2 p-1 sm:p-2 rounded bg-transparent text-black hover:scale-90 transition ease-out duration-50 text-xs sm:text-base"}`}
              onClick={() => setCurrentTab("supplierProductsTable")}
            >
              Products
            </button>
            <button
            ...
            <button
              className={`${currentTab === "supplierProjectsTable" ? "border-x-2 border-t-2 p-1 sm:p-2 rounded bg-gray-700 text-white text-xs sm:text-base" : "border-x-2 border-t-2 p-1 sm:p-2 rounded bg-transparent text-black hover:scale-90 transition ease-out duration-50 text-xs sm:text-base"}`}
              onClick={() => setCurrentTab("supplierProjectsTable")}
            >
              Projects
            </button>
          </div>
