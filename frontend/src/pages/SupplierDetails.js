...
const SupplierDetails = () => {
        ...
        if (newProjects.length > 0) {
            try {
                ...
                await Promise.all(newProjects.map(async projectId => {
                    ...
                    const projectRes = await fetch(`https://empire-pms.vercel.app/api/project/${projectId}`);
                    if (!projectRes.ok) {
                    ...
                    const updateRes = await fetch(`https://empire-pms.vercel.app/api/project/${projectId}`, {
                        method: 'PUT',
                        headers: {
    ...
    const fetchSupplierDetails = useCallback(async () => {
        try {
            const res = await fetch(`https://empire-pms.vercel.app/api/supplier/${id}`);
            if (!res.ok) {
    ...
    const fetchOrdersBySupplier = async () => {
        try{
            const res = await fetch(`https://empire-pms.vercel.app/api/order`);
            if (!res.ok) {
    ...
    useEffect(() => {
        const fetchSupplierProducts = async () => {
            try{
                const res = await fetch(`https://empire-pms.vercel.app/api/supplier/${id}/products`);
                if (!res.ok) {
